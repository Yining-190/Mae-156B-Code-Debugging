...
void loop() {
  if (Serial.available()) {
      ...
      if (angleStr.length() > 0) {  // Check if data is not empty
          float angle = angleStr.toFloat();  // Convert to float
          if (!isnan(angle)) {  // Check if the conversion was successful
              Serial.print("Received angle: ");
              Serial.println(angle);  // Echo back the received angle
          } else {
  ...
  Serial.begin(9600);  // Communication with Arduino A
...
void loop() {
  ...
  delay(50);  // Add a small delay (50 ms)
    ...
    if (Serial.available()) {
        ...
        if (angleStr.length() > 0) {  // Check if data is not empty
            ...
            if (!isnan(angle)) {  // Check if the conversion was successful
                ...
                Serial.println(angleStr);  // Echo back for debugging
            } else {
                Serial.println("Invalid data received");
            }
        }
